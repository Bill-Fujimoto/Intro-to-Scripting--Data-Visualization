...
def get_county_attributes(svg_file_name):
    ...
    """
    svg_data = minidom.parse(svg_file_name)  # parseString also exists
    path_strings = [(path.getAttribute('id'), path.getAttribute('d')) for path \
                    in svg_data.getElementsByTagName('path')]
...
def get_boundary_coordinates(boundary_data):
    ...
    Ignores 'M', 'L, 'z'
    """
    splits = boundary_data.strip().split(' ')
    boundary_pairs =[]
    for item in splits:
        if item != 'M' and item != 'L' and item != 'z':           
            x, y = item.split(',')
            boundary_pairs.append((float(x) , float(y.rstrip('M'))))
        else:
            continue
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    """
    svg_data = get_county_attributes(svg_file_name)
    ...
    with open(csv_file_name, 'w', newline='') as csv_file:
        ...
        for row in svg_data:
            coord_pairs = get_boundary_coordinates(row[1])
            x, y = compute_county_center(coord_pairs)
            new_row = [row[0], x, y]
